...
- [ ] 1. Tier 1 — Infrastructure & deploy
  ...
  - [ ] 1.4 Add Base Sepolia branch to `Web3Context.getExpectedNetworkConfig()` and `getReadProvider()`
  ...
  - [ ] 1.5 Write `scripts/seedDemo.js` (idempotent)
  ...
  - [ ] 1.6 Deploy contracts to Base Sepolia (`npm run deploy:base`)
  ...
  - [ ] 1.7 Run `npm run seed:base` and verify demo wallet manifest
...
- [ ] 2. Tier 1 — Role-based dashboards
  - [ ] 2.1 Build `OwnerDashboard.jsx`
  ...
  - [ ] 2.2 Build `InvestorDashboard.jsx` (with normal `claimAll` first; UGF wraps in 3.3)
  ...
  - [ ] 2.3 Wire role-aware routing and navbar in `App.jsx`
...
- [ ] 3. Tier 1 — UGF integration
  - [ ] 3.1 Install `@tychilabs/react-ugf` in frontend
  ...
  - [ ] 3.2 Build `UGFContext.jsx` skeleton and mount provider
  ...
  - [ ] 3.3 UGF-wrap `claimAll` in `InvestorDashboard.jsx`
  ...
  - [ ] 3.4 Add gas-in-Mock-USD badge and CSS
...
- [ ] 5. Tier 2 — Differentiators
  - [ ] 5.1 Generic `ugfExecute` helper finalized (5A.1)
  ...
  - [ ] 5.2 UGF-wrap `depositRental` in `OwnerDashboard.jsx` (5A.2)
  ...
  - [ ] 5.3 UGF-wrap `buyFromOwner` in `Property.jsx` (5A.3)
  ...
  - [ ] 5.4 UGF-wrap `buyFromListing` and `cancelListing` (5A.4)
  ...
  - [ ] 5.6 Activity feed — frontend `<ActivityFeed />` + `logTx` POST (5C frontend)
  ...
  - [ ] 5.7 Activity feed — backend validation (5C backend)
  ...
  - [ ] 5.8 Side-by-side cost banner `<CostBanner />` (5D)
  ...
  - [ ] 5.9 Rename "Dividends" → "Claim Rent" / "Rent History" (5E)
  ...
  - [ ] 5.10 In-app faucet helper `<FaucetPanel />` + backend `/api/faucet/usdc` (5F)
  ...
  - [ ] 5.11 Brand pass — name, logo, landing screen (5G)
...
- [ ] 6. Tier 3 — Stretch (optional)
  - [ ] 6.1 Embedded wallet via Privy (6A)
    - Install `@privy-io/react-auth` in `frontend/`.
    - Generalize `Web3Context` so `connect()` falls through to Privy when `window.ethereum` is undefined; `getSigner()` returns whichever provider produced a signer.
    - Verify a Privy-signed `claimAll` succeeds via `ugfExecute` once the wallet has `TYI_MOCK_USD` (faucet panel covers this).
    - **Files**: `frontend/package.json`, `frontend/src/context/Web3Context.jsx`, `frontend/src/main.jsx`.
    - **Depends on**: 5.12.
